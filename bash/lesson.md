...
Because terminal applications typically open to your home directory by default, the expected output is the absolute path of your *home directory*, which will vary based on your operating system and, of course, your account name. 
...
This command returns the *absolute path* of the current working directory. All relative file and folder paths used as arguments must be **relative to this current working directory**.
...
ls
```
The starting contents of your home directory will vary with your operating system, but a typical set of folders looks like this.
...
The `ls` command stands for **l**ist directory content**s**. It is also the first command that we will use with options. 

**Options** are letters signaled by a hyphen that are passed to commands to change the behavior of that command.

To use your first option, type `ls -l` with a <kbd>-</kbd> (hyphen) key. Options must come *after* the command they modify and *without space* between the hyphen and the letter representing the option.
...
```output
drwxr-xr-x 1 Erin 197121        0 Feb  4 11:27 Desktop/
drwxr-xr-x 1 Erin 197121        0 Nov 30 11:45 Documents/
drwxr-xr-x 1 Erin 197121        0 Dec 30 22:35 Downloads/
drwxr-xr-x 1 Erin 197121        0 Jan 31  2021 Favorites/
...
```

For example, this command enables "long-listing" format. As you can see, we see more detail about each of our files and folders. We'll come back to what each column in this new listing format means later when discussing permission.

You can enable multiple options simultaneously by putting both letters after the hyphen. 

Here, the `h` option stands for "human-readable" and prints file sizes KB, MB, and GB rather than in bytes for readability.
...
Inspect the contents and their respective using `ls -F`.
...
#### Quiz
{: .no_toc }
> Are these files or folders? How do you know? 

#### Answer
{: .no_toc }
> These are folders, as indicated by the `/` at the end.
...
We don't have to traverse through the filesystem one folder at a time!  Next, we'll move to the `talapas-bash/exercise-data/writing` directory and see what it contains.

### Auto-Complete with Bash
{: .no_toc }
For lengthy filepaths, the shell can infer file and folder names through auto-completion. Type `cd exer` and <kbd>Tab</kbd> and the shell will automatically complete it to `exercise-data`. 

However, if you press <kbd>Tab</kbd> and no paths populate, it means there's more than one possible path with the current prefix. Because `exercise-data/` has multiple files and subfolders inside, the shell can't decide path which to populate. Press <kbd>Tab</kbd> twice to see all the possibilities.
...
#### Answer
{: .no_toc }
> The command will file with the following error because there is no `exercise-data` directory inside `exercise-data/writing` and we supplied a relative path.
> ```output
...
#### Quiz
{: .no_toc }
> From your current working directory of `writing`, how could use `..` and `ls` to see the contents of the `mice` directory? **Hint**: the `mice` directory is `talapas-bash/exercise-data/mice`.
...
Before moving to the next task, navigate to the `writing` directory.
...
Return to the `talapas-bash/exercise-data/writing` directory,
...
{: .no_toc }
To look at long text files, let's practice with a screen reader called `less`.
...
It is important to include the `-r` flag. 
If you want to copy a directory and you omit this option you will see a message that the directory has been omitted because `-r not specified`.
...
```bash
mkdir journal
touch journal/day1.txt journal/day2.txt journal/day3.txt
```
...
-rw-r--r-- 1 Erin 197121 0 Feb  3 00:18 day3.txt
```
...
files from. The only hope in scenarios like this is to restore a version of the file or folder from a system backup. As researchers working in a shared computing environment, it is your responsibility not to use `rm` on project files and folders unless it is safe to remove them.
...
It is the shell, not the other programs, that expands the wildcards.
